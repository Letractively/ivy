...
public static function getInstance ()
{
	if (!isset(self::$instance)) {
		self::$instance = new Ivy_Template();
	}
...
{		
	(int) $i = 0;
	(string) $name = (empty($name) ? 'default' : $name);		
	...
	foreach ($dataArray['fieldSpec'] as $metaField => $metaArray) {	
		if (!isset($metaArray['front']['nobuild'])) {
			...
			if (!empty($fields) && !in_array($metaField, $fields)) {
				continue;
			}
			...
			switch ($metaArray['front']['type']) {
				case 'radio'	:
					if (isset($dataArray['data'][0]['_'.$metaField])) {
						$finalValue = $dataArray['data'][0]['_'.$metaField];
					} else {
						$finalValue = $dataArray['data'][0][$metaField];
					}
					$finalValue = trim($finalValue);
					break;
				case 'select'	:
					$finalValue = (isset($dataArray['data'][0]['_'.$metaField]) ? $dataArray['data'][0]['_'.$metaField] : $dataArray['data'][0][$metaField]);
					$finalValue = trim($finalValue);
					break;
				case 'checkbox'	:
					(array) $tempVar = array ();
					$tempVar = (isset($dataArray['data'][0]['_'.$metaField]) ? $dataArray['data'][0]['_'.$metaField] : $dataArray['data'][0][$metaField]);
...
public function addResult ($id, $name = 'default', $fields = array ())
{
	...
	if (isset($data['tableSpec']['search'])) {
		$data['tableSpec']['search'] = array_change_key_case($data['tableSpec']['search'], CASE_UPPER);
	}

	if (isset($data['data'])) {
		$data['fieldSpec'] = array_change_key_case($data['fieldSpec'], CASE_UPPER);
		...
		foreach ($data['data'] as $record => $array) {
			...
			foreach ($array as $field => $value) {
				...
				if (isset($data['tableSpec']['search'])) {
					...
					//print_r($data['tableSpec']['search']['icon']);
				...
				if ($field[0] != '_') {	
					 ...
					 */
					if (isset($data['fieldSpec'][$field])) { ## END ##
						...
						if ($data['tableSpec']['pk'][0] != $field && 							!isset($data['fieldSpec'][$field]['front']['noview'])) {
							...
							$this->data['result'][$name]['meta'][$field] = 							(isset($data['fieldSpec'][$field]) ? 
								$data['fieldSpec'][$field]['front'] : 
								$data['fieldSpec']['"' . $field . '"']['front']);
						}
			...
			if (isset($array['_' . $data['tableSpec']['pk'][0]])) {
				$this->data['result'][$name]['data'][$record]['_PK'] = 
					$array['_' . $data['tableSpec']['pk'][0]];						
				
				unset($this->data['result'][$name]['data']
					[$record][ '_' . $data['tableSpec']['pk'][0] ]);
				
				unset($this->data['result'][$name]['meta']
					[ $data['tableSpec']['pk'][0] ]);
			
			} else if (isset($array[$data['tableSpec']['pk'][0]])) {
				$this->data['result'][$name]['data'][$record]['_PK'] = 					$array[$data['tableSpec']['pk'][0]];
				
				if (isset($data['fieldSpec'][ $data['tableSpec']['pk'][0] ]
					['front']['show'])) {
					
					if ($data['fieldSpec'][ $data['tableSpec']['pk'][0] ]
						['front']['show'] != 'y') {
						
						unset($this->data['result'][$name]['data']
							[$record][ $data['tableSpec']['pk'][0] ]);
...
public function addData ($data, $name = 'default')
{
	...
	public function build ($template = null, $globalTemplate = null)
	{
		...
		if (isset($this->data['system']['script'])) {
			...
			foreach ($cachedScript as $key => $value) {
				#if 
		...
		if ($config['output']['debug'] == 1 && !isset($_GET['ivy_debug'])) {
			#$this->addScript('shared/ivy/script/jquery.jkpanel/jkpanel.js');
			#$this->addStyle('shared/ivy/script/jquery.jkpanel/default.css');
			/*
			$this->addStyle('shared/ivy/script/codascroll/css/default.css');
			$this->addStyle('shared/ivy/script/codascroll/css/highslide.css');
			
			$this->addScript('shared/ivy/script/codascroll/js/scrollto.js');
			$this->addScript('shared/ivy/script/codascroll/js/localscroll.js');
			$this->addScript('shared/ivy/script/codascroll/js/serialscroll.js');
			$this->addScript('shared/ivy/script/codascroll/js/coda.js');
			$this->addScript('shared/ivy/script/codascroll/js/highslide/highslide.js');
			$this->addScript('shared/ivy/script/codascroll/js/scrollto.js');
				*/
		...
		if ($template !== FALSE) {
			...
			if ($errorArray !== FALSE) {
				foreach ($errorArray as $key => $data) {
					$this->data['form']['default'][$key]['error'] = $data;
				}
			}
			...
			if (isset($_GET['ivy_rest'])) {
				...
				echo '<?xml-stylesheet type="text/xsl" href="http://pctnt07/xsl.xsl" ?>';
				


				echo '<root>' . ArrayToXML($this->data) . '</root>';
			
			} else if (isset($_GET['ivy_debug'])) {

				#echo '<div style="overflow:auto;height:285px;">';
				#echo '<pre>';
				#print_r($this->data);
				#echo '</pre>';
				#echo '</div>';
				$templateEngine->parse($template, $globalTemplate);
				...
				$templateEngine->parse($this->stylesheet, $globalTemplate);
			}
...
function g ($value, $key) {
	if (strstr('<![CDAT', $value) === false) {
		return '<![CDATA[' . $value . ']]>';
	}
}
